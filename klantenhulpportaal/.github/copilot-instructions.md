...
- **KEY RULE**: Use consistent typing with store types (`New<T>`, `Updatable<T>`) instead of creating duplicate form interfaces. Forms should use `Partial<New<T>>` for creation and `Partial<Updatable<T>>` for updates, eliminating the need for type casting and maintaining consistency with store actions
